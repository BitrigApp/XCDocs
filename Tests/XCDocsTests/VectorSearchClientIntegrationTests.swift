...
@Suite("VectorSearchClient Integration", .enabled(if: LiveEnvironment.isAvailable), .serialized) struct VectorSearchClientIntegrationTests {
    ...
    @Test func normalizesAndAppliesFrameworkFilters() async throws {
        let client = try makeClient()
        let vector = try await LiveEnvironment.embeddingVector(for: LiveEnvironment.searchQuery)
        let hits = try client.search(vector: vector, frameworks: ["  \(LiveEnvironment.searchFramework)  ", "", "   "], kinds: [], limit: 5, includeContent: false)
    ...
    @Test func normalizesAndAppliesKindFilters() async throws {
        let client = try makeClient()
        let vector = try await LiveEnvironment.embeddingVector(for: LiveEnvironment.searchQuery)
        let hits = try client.search(vector: vector, frameworks: [], kinds: ["  article  ", "", "   "], limit: 5, includeContent: false)
    ...
    @Test func includeContentControlsWhetherSearchResultsContainContent() async throws {
        ...
        let hitsWithoutContent = try client.search(vector: vector, frameworks: [], kinds: [], limit: 5, includeContent: false)
        let hitsWithContent = try client.search(vector: vector, frameworks: [], kinds: [], limit: 5, includeContent: true)
    ...
    @Test func returnsAnEmptyArrayWhenLimitIsZeroOrLess() throws {
        let client = try makeClient()
        let hits = try client.search(vector: Data(), frameworks: [LiveEnvironment.searchFramework], kinds: ["article"], limit: 0, includeContent: false)

        #expect(hits.isEmpty)
    }

    private func makeClient() throws -> VectorSearchClient { try VectorSearchClient(databaseDirectoryURL: LiveEnvironment.databaseDirectoryURL(), readOnly: true) }
}
