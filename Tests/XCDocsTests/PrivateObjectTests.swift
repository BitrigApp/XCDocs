...
struct PrivateObjectTests {
  ...
  func resolvesInstanceAndClassMethods() throws {
    ...
    let instanceMethod = try #require(
      wrapper.objcInstanceMethod(selector: instanceSelector, as: NumberGetter.self)
    )
    let classMethod = try #require(
      FixtureWrapper.objcClassMethod(
...
private final class KeyValueFixture: NSObject {
  ...
  dynamic var objectArray: [AnyObject] = [
    "first" as NSString,
    NSNumber(value: 2),
  ]

  dynamic func answer() -> NSNumber {
    NSNumber(value: 42)
  }

  class dynamic func classAnswer() -> NSNumber {
    NSNumber(value: 7)
  }
}
