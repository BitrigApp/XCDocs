...
package enum LiveEnvironment {
    ...
    private static let documentationAssetRootPath = "/System/Library/AssetsV2/com_apple_MobileAsset_AppleDeveloperDocumentation"

    package static var isAvailable: Bool {
        let fileManager = FileManager.default
        return fileManager.fileExists(atPath: mediaAnalysisServicesPath) && fileManager.fileExists(atPath: vectorSearchPath) && fileManager.fileExists(atPath: documentationAssetRootPath) && ((try? DocumentationAssetLocator().locateDatabaseDirectoryURL()) != nil)
    }

    package static func databaseDirectoryURL() throws -> URL { try DocumentationAssetLocator().locateDatabaseDirectoryURL() }
