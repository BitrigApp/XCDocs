...
package struct VSKDisjunctiveFilterObject: PrivateObject {
    ...
    package init(operatorRawValue: Int64, value: VSKDatabaseValueObject) throws {
        ...
        guard let initialize = VSKDisjunctiveFilterObject(base: rawObject).objcInstanceMethod(
            selector: Self.initSelector,
            as: VSKDisjunctiveFilterInitMethod.self
        ) else {
            throw BridgeError(.selectorUnavailable, "Missing -initWithOperator:value: on VSKDisjunctiveFilter")
        }
...
private typealias VSKDisjunctiveFilterInitMethod =
    @convention(c) (AnyObject, Selector, Int64, AnyObject) -> AnyObject?
