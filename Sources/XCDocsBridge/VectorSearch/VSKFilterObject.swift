...
package struct VSKFilterObject: PrivateObject {
    ...
    package init(attribute: VSKAttributeObject, disjunctiveFilters: [VSKDisjunctiveFilterObject]) throws {
        ...
        guard let initialize = VSKFilterObject(base: rawObject).objcInstanceMethod(selector: Self.initSelector, as: VSKFilterInitMethod.self) else { throw BridgeError(.selectorUnavailable, "Missing -initWithAttribute:disjunctiveFilters: on VSKFilter") }

        guard let object = initialize(rawObject, Self.initSelector, attribute.base, disjunctiveFilters.map(\.base) as NSArray) else { throw BridgeError(.operationFailed, "Failed to create VSKFilter") }
