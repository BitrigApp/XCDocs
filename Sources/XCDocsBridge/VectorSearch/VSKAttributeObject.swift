...
package struct VSKAttributeObject: PrivateObject {
  ...
  init(name: String, columnType: VSKColumnTypeObject) throws {
    ...
    guard
      ...
      throw BridgeError(.selectorUnavailable, "Missing -initWithName:columnType: on VSKAttribute")
    }

    guard let object = initialize(rawObject, Self.initSelector, name as NSString, columnType.base)
    else {
  ...
  package var name: String {
    guard
      let getter = objcInstanceMethod(
        selector: Self.getNameSelector,
        as: VSKAttributeGetNameMethod.self
      )
    else {
      return ""
    }
...
private typealias VSKAttributeInitMethod =
  @convention(c) (AnyObject, Selector, NSString, AnyObject) -> AnyObject?

private typealias VSKAttributeGetNameMethod =
