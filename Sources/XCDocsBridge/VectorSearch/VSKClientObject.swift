...
package final class VSKClientObject: PrivateObject {
    ...
    package init(config: VSKConfigObject) throws {
        ...
        guard let initialize = VSKClientObject(base: rawObject).objcInstanceMethod(
            selector: Self.initSelector,
            as: VSKClientInitMethod.self
        ) else {
            throw BridgeError(.selectorUnavailable, "Missing -initWithConfig:error: on VSKClient")
        }
    ...
    package func search(
        ...
        guard let method = objcInstanceMethod(
            selector: Self.searchByVectorSelector,
            as: VSKSearchByVectorMethod.self
        ) else {
            throw BridgeError(.selectorUnavailable, "Missing search selector on VSKClient")
        }
    ...
    package func stringIdentifiedAssets(
        ...
        guard let method = objcInstanceMethod(
            ...
            throw BridgeError(.selectorUnavailable, "Missing stringIdentifiedAssets selector on VSKClient")
        }
