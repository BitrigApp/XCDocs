...
package struct VSKDatabaseValueObject: PrivateObject {
    ...
    package init(string: String) throws {
        ...
        else { throw BridgeError(.selectorUnavailable, "Missing -initWithStringValue: on VSKDatabaseValue") }

        guard let object = initialize(rawObject, Self.initWithStringSelector, string as NSString) else {
    ...
    var stringValue: String? {
        guard
            let getter = objcInstanceMethod(selector: Self.getStringSelector, as: VSKDatabaseValueGetStringMethod.self)
        else { return nil }
