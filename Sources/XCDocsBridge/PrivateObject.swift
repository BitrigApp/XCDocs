...
extension NSObject {
  fileprivate func xcdocsValue(forKey key: String) -> Any? {
    value(forKey: key)
  }
}
...
extension PrivateObject {
  func value<T>(forKey key: String, as type: T.Type) -> T? {
    (base as? NSObject)?.xcdocsValue(forKey: key) as? T
  }

  func value<T>(forKey key: Key, as type: T.Type) -> T? {
    value(forKey: key.rawValue, as: type)
  }
  ...
  func array(forKey key: Key) -> [AnyObject] {
  ...
  static func allocateObject(of cls: AnyClass, className: String) throws -> AnyObject {
    guard
      let alloc: AllocMethod = objcClassMethod(
        cls,
        selector: xcdocsAllocSelector,
        as: AllocMethod.self
      )
    else {
